...
/**
 * 专门用于物理清理走势图最末端的“总值”干扰列
 * @param {HTMLTableElement} table - 要清洗的表格对象
 * @param {number} expectedRedCols - 预期的号码列数（如35或33）
 */
function removeTotalValueColumn(table, expectedRedCols) {
  if (!table)
    return

  // 核心逻辑：直接遍历所有 tr，如果单元格数量多于 (1期号 + 预期号码列数)，
  // 说明末尾多出了“总值”或其他无用列，强行砍掉。
  const targetLength = 1 + expectedRedCols

  table.querySelectorAll('tr').forEach((tr) => {
    // 只有数据行、表头行才需要检查，跳过某些极短的标题行
    if (tr.children.length > targetLength) {
      // 物理删除尾部多余的 td/th，直到长度符合预期
      while (tr.children.length > targetLength) {
        tr.lastElementChild.remove()
      }
    }
  })

  // 同时清理表头的 colspan，防止原网页表格因colspan未更新而崩塌
  const headerCells = table.querySelectorAll('thead th[colspan]')
  headerCells.forEach((th) => {
    if (th.getAttribute('colspan') > expectedRedCols) {
      th.setAttribute('colspan', expectedRedCols)
    }
  })
}
...
function removeAwardAndBlueZoneColumns() {
  ...
  removeTotalValueColumn(table, redCols)
